...
open PRIVATE, "< $datadir/PRIVATE.tsv";
while (<PRIVATE>) {
  s/\r?\n//;
...
{
  open SCANSITE, "< $datadir/scansite_matrix.tsv";
  $_ = <SCANSITE>;
  while (<SCANSITE>) {
    s/\r?\n//;
